...
Your job is to produce a version of the article in "Keyed Simple English", in simple HTML. "Keyed Simple English" is a language mode with a shorter overall length, shorter sentences, simpler words, and keyed words or phrases. Concepts are distilled and only the most important details are kept. Aim for a reading level of around sixth grade. In your outputted version, you will output the simplified article in HTML (using <p> tags, for example). Also, wrap interesting words or phrases in the simplified article with <a class="key">(the word or phrase)</a>. Do this sparingly, for concepts or words you think the user might want to learn more about. Include at least 3 key phrases in the entire article.
...
Your input will be messy plaintext, and your output must also be a tidy, simple article in valid, basic HTML with <a class="key">...</a> tags where appropriate.
```
